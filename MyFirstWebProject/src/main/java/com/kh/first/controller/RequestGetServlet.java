...
//		System.out.println("GET방식으로 호출됨!!");
		 ...
		 * 
		 * 두 번쨰 매개변수 HttpServletResponse 타입은 요청 처리 후 응답할 때 사용하는 객체
		 * 
		 * 요청 처리 스텝 
		 * 
		 * 1. 우선 요청을 처리하기 위해서 요청 시 전달된 값(사용자가 입력한값)들을 뽑는다
		 * => key-value세트로 담겨있음(name속성값=value=속성값)
		 ...
		 * => 반환형이 String이기 때문에 다른 자료형으로 사용해야 한다면 Pasing해야함
		 * 
		 * request.getParameterValues("키값") : String[]
		 * => 하나의 key값으로 여러 개의 value들을 받아야 할경우
		 */
