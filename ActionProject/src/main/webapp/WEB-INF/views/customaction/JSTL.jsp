<%@ page language="java" contentType="text/html; charset=UTF-8"
   ...
   <!-- 
   	URI(Uniform Resousrce Identfier)
   	식별자 - 고유한 이름 (PK) -> 실제위치가 아니라 논리적인 식별값
   	
   	URL(Uniform Resuource Loactor)
...
<body>
	...
	<pre>
		솔직하게 이야기하곘습니다. 사실 그냥 아까배운 Scope들에 새로운 속성 추가하는 방법
	...
	<!-- value속성 말고 시작태그와 종료태그 사이에도 대입할 값을 기술 할 수 있음 -->

	<hr>
	
	<pre>
		* 속성 삭제(&lt;c:removce var="제거하려고하는 키값" scope="스코프영역(생략가능)" />)
	...
	<pre>
		* 속성 출력(&lt;c:out value="출력할 값" default="기본값" escapeXml="t/f"/>)
		...
		- default : 기본값, value속성에 출력하고자 하는 값이 없을 경우 대체해서 출력할 
		            내용물을 쓸 수 있음(생략가능)
		- escapeXml : HTML 태그를 인식시킬수도있음
	</pre>
	
	num1을 출력! : <c:out value="${ num1 }" /> <br>
	num2을 출력! : <c:out value="${ num2 }" /> <br>
	이걸 왜 이렇게 씀? : ${ num1 } <br>
	...
	<pre>
		...
		&lt;c:forTokens var="값을보관할속성명" items="분리하고자하는 문자열" delims="구분자">
			반복적으로 실행 할 문구(출력만)
		&lt;/c:forTokens>
