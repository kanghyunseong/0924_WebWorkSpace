...
<body>
	...
	<pre>
		학원명 : <%= academy %>
		강의장 : <%= classRoom %>
		강사 정보 : <%= lecture.getName() %>, <%= lecture.getAge() %>, <%= lecture.getAddress() %>
		수강생의 정보
		<ul>
	...
	<p>없는 키값을 출력하는 경우 </p>
	<p>출력식을 사용함 : &lt;%= num %></p>
	<p>EL구문을 사용함 : ${ wadfasdfadf }</p>
	
	<h3>3. EL 사용 시 키값이 동일한 경우</h3>
	
	key 라는 키값에 담기 밸류 : ${ key } <br>
	...
	<!-- 
	 ...
	 <p>
	 	...
	 	big이 small 보다 크거나 같니? : ${ big >= small} 또는 ${ big ge small }<br>
	 	big이 small 보다 작거나 같니?? : ${ big <= small} 또는 ${ big le small }<br>
	 </p>
	 ...
	 <p>
	 	big과 small이 같습니까? : ${ big == samll } 또는 ${ big eq small } <br>
